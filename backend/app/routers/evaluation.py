...
def _fetch_all_chats() -> List[dict]:
    ...
    for r in results:
        chats.append({
            ...
            "balance": json.loads(r["balance"]) if r.get("balance") else {},
        })
    return chats


def _compute_automated_metrics(chat: dict) -> AutomatedMetrics:
    """Compute automated metrics from a single chat's stored data."""
    chat_id = chat["id"]
    citations = chat.get("citations", [])
    experts = chat.get("experts", [])
    balance = chat.get("balance", {})
    ...
    party_coverage = parties_represented / ALL_PARTIES if ALL_PARTIES > 0 else 0
    ...
    # 3. Balance Score
    bias = balance.get("bias_score", 0.5)
    balance_score = max(0, 1.0 - bias)
    magg_pct = balance.get("maggioranza_percentage", 0)
    opp_pct = balance.get("opposizione_percentage", 0)

    # 4. Authority Utilization
    auth_scores = [
        e.get("authority_score", 0) or e.get("total_score", 0)
        for e in experts
        if e.get("authority_score") or e.get("total_score")
    ]
    avg_auth = sum(auth_scores) / max(len(auth_scores), 1) if auth_scores else 0

    # 5. Response Completeness (count ## section headers)
    sections = re.findall(r"^##\s+", answer, re.MULTILINE)
    completeness = min(len(sections) / ALL_PARTIES, 1.0)
    ...
    return AutomatedMetrics(
        ...
        maggioranza_pct=round(magg_pct, 1),
        opposizione_pct=round(opp_pct, 1),
...
def _compute_aggregated(metrics_list: List[AutomatedMetrics]) -> AggregatedMetrics:
    ...
    if n == 0:
        return AggregatedMetrics(
            ...
            avg_balance_score=0, avg_authority_utilization=0,
            avg_response_completeness=0,
            ci_party_coverage=(0, 0), ci_citation_integrity=(0, 0),
            ci_balance_score=(0, 0), ci_authority_utilization=(0, 0),
            ci_response_completeness=(0, 0),
        )

    pc = [m.party_coverage_score for m in metrics_list]
    ci = [m.citation_integrity_score for m in metrics_list]
    bs = [m.balance_score for m in metrics_list]
    ...
    return AggregatedMetrics(
        ...
        avg_citation_integrity=round(sum(ci) / n, 4),
        avg_balance_score=round(sum(bs) / n, 4),
        avg_authority_utilization=round(sum(au) / n, 4),
        avg_response_completeness=round(sum(rc) / n, 4),
        ci_party_coverage=_compute_ci(pc),
        ci_citation_integrity=_compute_ci(ci),
        ci_balance_score=_compute_ci(bs),
...
async def get_dashboard():
    ...
    for chat in chats:
        try:
            metrics_map[chat["id"]] = _compute_automated_metrics(chat)
...
async def get_chat_metrics(chat_id: str):
    ...
    result = client.query("""
        ...
        RETURN c.id AS id, c.query AS query, c.answer AS answer,
               c.timestamp AS timestamp, c.citations AS citations,
               c.experts AS experts, c.balance AS balance
    """, {"id": chat_id})
    ...
    return _compute_automated_metrics(chat)
...
async def export_csv():
    ...
    writer.writerow([
        ...
        # Automated metrics
        "party_coverage", "citation_integrity", "balance_score",
    ...
    for chat in chats:
        try:
            m = _compute_automated_metrics(chat)
        except Exception:
            continue
